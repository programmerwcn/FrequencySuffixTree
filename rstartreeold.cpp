...
void RTree::rangeQuery(vector<vector<Object>> &objects, const Rectangle &range,
                       uintptr_t id)
{
    Node *node = (Node *)storage->read(id);
    unsigned i;
    for(i = 0; i < node->numChildren; i++)
        ...
        {
            if(node->isLeaf()) {
                vector<Object> object_group;
                object_group.push_back(node->objects[i]);
                objects.push_back(object_group);
            } else {
                rangeQuery(objects, range, node->objects[i].id);
            }
        }
...
void RTree::insertSpatialObjectInnerNode(SpatialObject &object, uintptr_t id) {
    /* 查找所有包含object定位的节点 */
    Node *node = (Node *)storage->read(id);
    //  如果当前节点包含定位，更新它对应的suffix tree，然后继续查找子节点；否则停止
    if (node->mbr.contains(object.mbr)) {
        /* suffix tree更新 */
        // read suffix tree
        SuffixTree stree;
        if (node->suffixId == 0) {  // 之前并没有存过这个suffix tree
          node->suffixId = (++SuffixTree::maxId);
        } else {
            stree.load("/Users/christine/Documents/Study/Grade 4/大四上/lab/untitled/suffix-tree/" + to_string(node->suffixId));
        }
        // insert data
        stree.add_string(object.documents);
        stree.dump_tree();
        // write suffix tree
        stree.save("/Users/christine/Documents/Study/Grade 4/大四上/lab/untitled/suffix-tree/" + to_string(node->suffixId));

        /* 继续查找子节点 */
        if (!node->isLeaf()) {
            for (int i = 0; i < node->numChildren; i++) {
                insertSpatialObjectInnerNode(object, node->objects[i].id);
            }
        } else {
            // 在object上存储suffix tree
            for (int i = 0; i < node->numChildren; i++) {
                Object & node_object = node->objects[i];
                if (object.mbr.contains(node_object.mbr)) {
                    /* suffix tree更新 */
                    SuffixTree stree;
                    node_object.suffixId = (++SuffixTree::maxId);
                    // insert data
                    stree.add_string(object.documents);
                    stree.dump_tree();
                    // write suffix tree
                    stree.save("/Users/christine/Documents/Study/Grade 4/大四上/lab/untitled/suffix-tree/" + to_string(node_object.suffixId));
                }
            }
        }
    }
...
void RTree::insertSpatialObjectInnerNode(SpatialObject &object) {
    uintptr_t id = storage->getRoot();
    insertSpatialObjectInnerNode(object, id);
}


void RTree::insertSpatialObjectLeafNode(SpatialObject &object) {
    uintptr_t id = storage->getRoot();
    insertSpatialObjectLeafNode(object, id);
}

void RTree::insertSpatialObjectLeafNode(SpatialObject &object, uintptr_t id) {
/* 查找所有包含object定位的节点 */
    Node *node = (Node *)storage->read(id);
    //  如果当前节点包含定位，继续查找子节点；否则停止
    if (node->mbr.contains(object.mbr)) {
        if (!node->isLeaf()) {
            for (int i = 0; i < node->numChildren; i++) {
                insertSpatialObjectLeafNode(object, node->objects[i].id);
            }
        } else {
            // 在object上存储suffix tree
            for (int i = 0; i < node->numChildren; i++) {
                ...
                if (object.mbr.contains(node_object.mbr)) {
                    ...
                    stree.dump_tree();
                    // write suffix tree
                    stree.save("/Users/christine/Documents/Study/Grade 4/大四上/lab/untitled/suffix-tree-leaf/" + to_string(node_object.suffixId));
                }
            }
        }
    }
...
void RTree::rangeQueryInnerNodes(vector<Object> &objects, const Rectangle &range, uintptr_t id) {
    Node *node = (Node *)storage->read(id);
    unsigned i;
    if (range.contains(node->mbr)) {
        Object node_object{};
        node_object.id = id;
        node_object.mbr = node->mbr;
        node_object.suffixId = node->suffixId;
        objects.push_back(node_object);
    } else {
        for (i = 0; i < node->numChildren; i++) {
            if (node->objects[i].mbr.intersects(range)) {
                if (node->isLeaf())
                    ...
                    rangeQueryInnerNodes(objects, range, node->objects[i].id);
            }
        }
    }
    storage->free(node);
...
void RTree::rangeQueryInnerNodes(vector<vector<Object>> &objects, const Rectangle &range, uintptr_t id) {
    Node *node = (Node *)storage->read(id);
    unsigned i;
    if (range.contains(node->mbr)) {
        Object node_object{};
        vector<Object> leaf_nodes;
        rangeQuery(leaf_nodes, node->mbr);
        objects.push_back(leaf_nodes);
    } else {
        for (i = 0; i < node->numChildren; i++) {
            if (node->objects[i].mbr.intersects(range)) {
                if (node->isLeaf()) {
                    vector<Object> leaf_nodes;
                    leaf_nodes.push_back(node->objects[i]);
                    objects.push_back(leaf_nodes);
                }
                else
                    rangeQueryInnerNodes(objects, range, node->objects[i].id);
...
void RTree::rangeQueryInnerNodes(vector<Object> &objects, const Rectangle &range) {
    objects.clear();
    uintptr_t id = storage->getRoot();
    rangeQueryInnerNodes(objects, range, id);
}

void RTree::rangeQueryInnerNodes(vector<vector<Object>> &objects, const Rectangle &range) {
    objects.clear();
    uintptr_t id = storage->getRoot();
    rangeQueryInnerNodes(objects, range, id);
}
...
int RTree::get_height() {
    uintptr_t id = storage->getRoot();
    Node *node = (Node *)storage->read(id);
    int height = 1;
    while (!node->isLeaf()) {
        height ++;
        id = node->objects[0].id;
        node = (Node *)storage->read(id);
    }
    return height;
}
